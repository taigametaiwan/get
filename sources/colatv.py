...
PLAYER_ORIGIN_FALLBACK = "https://colatv77.live"
PROJECT_ROOT = Path(__file__).resolve().parents[1]
OUTPUT_M3U = PROJECT_ROOT / "colatv_live.m3u"
OUTPUT_PIPE_M3U = PROJECT_ROOT / "colatv_live_pipe.m3u"
OUTPUT_VLC_M3U = PROJECT_ROOT / "colatv_live_vlc.m3u"
LEGACY_GIT_PLAYLIST_PATH = "colatv/colatv_live.m3u"
...
HOME_URLS = read_env_urls("COLATV_HOME_URLS", DEFAULT_HOME_URLS)
TARGET_URL = HOME_URLS[0]


CONCURRENCY_LIMIT = read_env_int(
    "COLATV_MATCH_CONCURRENCY", 4, minimum=1, maximum=12
)
HOME_WAIT_MS = read_env_int(
    "COLATV_HOME_WAIT_MS", 6000, minimum=1000, maximum=30000
)
STREAM_WAIT_SECONDS = read_env_int(
    "COLATV_ROOM_WAIT_SECONDS", 20, minimum=5, maximum=120
)
EXTRA_WAIT_AFTER_FIRST_STREAM = 5.0
FULL_SCAN = read_env_bool("COLATV_FULL_SCAN", True)
VERIFY_STREAMS = read_env_bool("COLATV_VERIFY_STREAMS", True)
VERIFY_TIMEOUT_SECONDS = read_env_int("COLATV_VERIFY_TIMEOUT_SECONDS", 8, minimum=3, maximum=20)
MAX_VERIFY_CANDIDATES = read_env_int("COLATV_MAX_VERIFY_CANDIDATES", 6, minimum=2, maximum=12)
...
KEEP_PREVIOUS_UNVERIFIED = read_env_bool("COLATV_KEEP_PREVIOUS_UNVERIFIED", False)
UPCOMING_FAR_THRESHOLD_MINUTES = read_env_int("COLATV_UPCOMING_FAR_THRESHOLD_MINUTES", 45, minimum=5, maximum=240)
UPCOMING_FAR_WAIT_SECONDS = read_env_int("COLATV_UPCOMING_FAR_WAIT_SECONDS", 7, minimum=3, maximum=30)
UPCOMING_NEAR_WAIT_SECONDS = read_env_int("COLATV_UPCOMING_NEAR_WAIT_SECONDS", 12, minimum=5, maximum=60)
HYBRID_HTTP_FIRST = read_env_bool("COLATV_HYBRID_HTTP_FIRST", True)
HTTP_DISCOVERY_TIMEOUT_SECONDS = read_env_int("COLATV_HTTP_DISCOVERY_TIMEOUT_SECONDS", 8, minimum=3, maximum=20)
HTTP_DISCOVERY_MAX_FOLLOWS = read_env_int("COLATV_HTTP_DISCOVERY_MAX_FOLLOWS", 4, minimum=1, maximum=10)
DELTA_SCAN_ENABLED = read_env_bool("COLATV_DELTA_SCAN_ENABLED", True)
DELTA_NEAR_MINUTES = read_env_int("COLATV_DELTA_NEAR_MINUTES", 45, minimum=5, maximum=180)
...
def _parse_previous_playlist_text(text: str, source_label: str) -> dict[str, list[dict[str, str]]]:
    ...
    for raw_line in (text or "").splitlines():
        ...
        if line.startswith("#EXTINF"):
            id_match = re.search(r'tvg-id="colatv-(\d+)-\d+"', line)
...
def normalize_playback_referer(value: str) -> str:
    """Giữ nguyên URL trang trận ColaTV làm Referer khi đã bắt được từ browser.

    Request mẫu của CDN ``meung.app`` dùng chính URL ``/truc-tiep/...`` làm
    Referer. Chỉ rơi về trang chủ khi scanner không thu được Referer hợp lệ.
    """
...
def clean_match_name(value: str, fallback_url: str) -> str:
    ...
    text = re.sub(
        r"(?i)\b(xem ngay|trực tiếp|hot|live|bóng đá|sắp diễn ra|đang diễn ra|colatv)\b",
        " ",
        text,
    )
    ...
    if not re.search(r"\bvs\b", text, re.I):
        ...
        # ColaTV: <doi-a>-vs-<doi-b>-luc-HHMM-ngay-DD-MM-YYYY-<id-ngau-nhien>
        slug = re.sub(
...
def derive_match_info(
    ...
    if not time_str:
        cola_schedule = re.search(
            r"-luc-(\d{2})(\d{2})-ngay-(\d{2})-(\d{2})-(\d{4})(?:-[a-z0-9]{6,})?/?$",
            parsed.path,
            re.I,
        )
        if cola_schedule:
            time_str = f"{cola_schedule.group(1)}:{cola_schedule.group(2)}"
        else:
...
async def collect_home_links(context: BrowserContext, home_url: str = TARGET_URL) -> list[dict[str, Any]]:
    ...
    print(f"👉 Đang mở trang chủ ColaTV: {home_url}")
...
async def collect_home_links_with_failover(context: BrowserContext) -> list[dict[str, Any]]:
    ...
    for home_url in HOME_URLS:
        ...
        if links:
            print(f"✅ Chọn miền ColaTV: {home_url} | trận={len(links)}", flush=True)
            return links
        print(f"⚠️ Miền ColaTV không có card trận, thử miền tiếp theo: {home_url}", flush=True)
    print("❌ Không miền ColaTV nào trả được card trận: " + ", ".join(f"{url}={count}" for url, count in attempts), flush=True)
    return []
...
def write_outputs(results: list[dict[str, Any]]) -> tuple[int, int]:
    """
    Tạo 3 playlist:
      - colatv_live.m3u: playlist phổ thông, URL nguyên bản + EXTHTTP/EXTVLCOPT.
      - colatv_live_pipe.m3u: biến thể Kodi-style URL|Header=Value.
      - colatv_live_vlc.m3u: URL nguyên bản + EXTVLCOPT dành riêng VLC.
    ...
    for result in sorted_results:
        ...
        match_name = result.get("match_name") or result.get("raw_title") or "ColaTV"
...
async def main() -> None:
    ...
    print(f"🥷 KHỞI ĐỘNG COLATV STREAM SCANNER - {SCANNER_VERSION}", flush=True)
    print(
        "ℹ️ Lệnh test riêng một trận (chỉ là hướng dẫn, không tự chạy):\n"
        '   python sources/colatv.py "URL_TRẬN_COLATV"'
    )
    print(
    ...
    async with async_playwright() as playwright:
        ...
        if DELTA_SCAN_ENABLED:
            ...
            save_delta_state(STATE_PATH, delta_state, "colatv")
