...
SOURCES = {
    ...
    "gavang": SourceConfig(
        key="gavang",
        label="Gà Vàng",
        script=ROOT / "sources" / "gavang.py",
        universal=ROOT / "gavang_live.m3u",
        pipe=ROOT / "gavang_live_pipe.m3u",
        vlc=ROOT / "gavang_live_vlc.m3u",
    ...
    "colatv": SourceConfig(
        key="colatv",
        label="ColaTV",
        script=ROOT / "sources" / "colatv.py",
        universal=ROOT / "colatv_live.m3u",
        pipe=ROOT / "colatv_live_pipe.m3u",
        vlc=ROOT / "colatv_live_vlc.m3u",
...
def source_playlist_paths(config: SourceConfig) -> tuple[Path, Path, Path]:
    return (config.universal, config.pipe, config.vlc)


def clear_source_playlists(config: SourceConfig) -> None:
    """Xóa playlist đã checkout từ lần chạy cũ để không giữ nhầm link chết."""
    for path in source_playlist_paths(config):
        try:
            path.unlink()
...
def ensure_source_playlists(config: SourceConfig, force_empty: bool = False) -> None:
    """Bảo đảm scanner luôn có đủ ba playlist tạm ở thư mục gốc."""
    for path in source_playlist_paths(config):
        path.parent.mkdir(parents=True, exist_ok=True)
        if force_empty or not path.exists():
...
def main() -> int:
    ...
    if not args.merge_only:
        for key in selected_keys:
            config = SOURCES[key]
            returncode, _fresh, _elapsed = statuses[key]
            ensure_source_playlists(config, force_empty=returncode != 0)
